    ...
    public class Common
    {
        ...
        public static bool CanCastLikeMonk(string name, WoWUnit unit)
        {
            ...
            if (spell.IsMeleeSpell)
            {
                ...
                {
                    Logger.WriteDebug("CanCast: cannot cast wowSpell {0} @ {1:F1} yds", spell.Name, unit.Distance);
                    return false;
                }
            }
            ...
            {
                Logger.WriteDebug("CanCast: wowSpell {0} requires {1} power but only {2} available", spell.Name, spell.PowerCost, Me.CurrentMana);
                return false;
            }

            if (Me.IsMoving && spell.CastTime > 0)
            {
                Logger.WriteDebug("CanCast: wowSpell {0} is not instant ({1} ms cast time) and we are moving", spell.Name, spell.CastTime);
                return false;
            }
